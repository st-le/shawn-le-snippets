...
int CvMatShow(const CvMat* matM)
{
	Mat mtxMat = Mat(matM,false);
...
//	A = Mat(1, 5, CV_32FC1, &data, 2);   // init from float 1D - array
	A = Mat(2, 4, CV_32F, data2D1);     //  init from 2D - array using CV_32F
	...
	//int sz[] = {5, 5, 5};
	//Mat A = Mat(3, sz, CV_32FC1, Scalar::all(1));
...
void CvMatVSMat()
{
	...
	*/
	...
	/*
	Mat mtxMat = Mat(2, 4, CV_32F, data2D1);
	CvMat mtxCvMat = mtxMat;
	Mat mtx = Mat(&mtxCvMat, false);
	MATmtxdisp(mtx);
	getchar();
	*/

	/* display CvMat with right-here CvMat to Mat conversion
	Mat mtxMat = Mat(2, 4, CV_32F, data2D1);
	CvMat mtxCvMat = mtxMat;
	CvMat *mtxCvMatPtr = &mtxCvMat;
	Mat mtx = Mat(mtxCvMatPtr, false);
	MATmtxdisp(mtx);
	getchar();
	*/

	/* using function cvMatShow
	Mat mtxMat = Mat(2, 4, CV_32F, data2D1);
	CvMat mtxCvMat = mtxMat;
	CvMat *mtxCvMatPtr = &mtxCvMat;
	CvMatShow(mtxCvMatPtr);
	getchar();
	*/

	/*
	// CvMat originates from array data, not from Mat conversion
	CvMat mtxCvMat = cvMat(2,4, CV_32F, data2D1);
	CvMat *mtxCvMatPtr = &mtxCvMat;
	CvMatShow(mtxCvMatPtr);
	getchar();
	*/

	// what about multi channel data
	CvMat mtxCvMat = cvMat(1,4, CV_32FC2, data2D1);
	CvMat *mtxCvMatPtr = &mtxCvMat;
	CvMatShow(mtxCvMatPtr);
	getchar();
	// -> all types of displaying CvMat & CvMat to Mat conversion work OK
