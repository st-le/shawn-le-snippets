// v1.0 24/09/12 my version, using v2.4 feature classes in nonfree
/*
...
int main(int argc, char** argv)
{
	...
	Mat pnts = Mat::zeros(1, keypoints_obj.size(), CV_32FC2);
	
	for (int i = 0; i < keypoints_obj.size() ; i++)
	{
		printf("(%0.2f, %0.2f)\n", keypoints_obj[i].pt.x, keypoints_obj[i].pt.y);
		pnts.at<Vec2f>(i)[0] = keypoints_obj[i].pt.x;
		pnts.at<Vec2f>(i)[1] = keypoints_obj[i].pt.y;
	}
	
	drawCircles(img, pnts);
