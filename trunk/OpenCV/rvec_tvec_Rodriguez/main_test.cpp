...
{	float ele;  // double will not work
...
//	A = Mat(1, 5, CV_32FC1, &data);   // init from float 1D - array
...
int main( int argc, const char** argv )
{
	...
	Mat CPose = Mat::zeros(6,1,CV_64FC1);    
	CPose.at<double>(0) = 10;
	CPose.at<double>(1) = 10;
	CPose.at<double>(2) = 10;
	CPose.at<double>(3) = CV_PI/2;
	CPose.at<double>(4) = -CV_PI/3;
	CPose.at<double>(5) = CV_PI/4;

	Mat rvec = CPose(Range(3,6), Range(0,1));
	//cout << rvec << endl;
	Mat tvec = CPose(Range(0,3), Range(0,1));
	// approach 1
	Rodrigues(rvec,rmat);
	//cout << rmat << endl;
	bTp1a = RT2Hom(rmat, tvec);

	// approach 2
	rmat = rotz(rvec.at<double>(2))*roty(rvec.at<double>(1))*rotx(rvec.at<double>(0));
	//cout << rmat << endl;
	bTp1b = transl(tvec)*rmat;

	// approach 3
	rmat = rotx(rvec.at<double>(0))*roty(rvec.at<double>(1))*rotz(rvec.at<double>(2));
	//cout << rmat << endl;
	Mat bTp1c = transl(tvec)*rmat;

	cout << CPose << endl << endl;

	cout << bTp1a << endl << endl;

	cout << bTp1b << endl << endl;

	cout << bTp1c << endl << endl;

	printf("CONCLUSION: rodrigues & trans*rz*ry*rz & trans*rx*ry*rz are not the same!");
