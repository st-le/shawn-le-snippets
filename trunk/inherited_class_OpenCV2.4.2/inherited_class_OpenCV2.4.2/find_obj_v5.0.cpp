...
// v4.0 27/09/12 add mouse event to solve init problem & ROI
// v4.2 27/09/12 rectangle approx of mouse selected region
// v5.0 27/09/12 using ROI
...
public: 
	void show()
	{
		printf("you've called to an inherited class!!");
	};
};
...
public:
	...
	float plus(float var1, float var2)
	{
		return var1+var2;
	}
...
private:
	...
	float minus(float var1, float var2)
	{
		return var1-var2;
	}
};

class mySmallClass : myBigClass
{
...
//	char *imgfile1 = "box_in_scene.png";
	...
	img.copyTo(markerMask);
	markerMask = Scalar::all(0);

	imshow("object selection", img);
	setMouseCallback( "object selection", onMouse, 0 );
	// #######
	...
	*/
	...
	myMat abc; // = myMat(1, 3, CV_32F, Scalar::all(15));
...
// an inherited class cannot inherit father class constructor?!
//      child class can inherit what from parents?? -> read this later